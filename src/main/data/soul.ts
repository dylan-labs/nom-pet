import type { Mood, SoulKernel, SoulPreset } from '../../shared/types';
 ...
 *
 * mode='line'    — bubble dialogue: one line, ≤15 chars, no emoji.
 * mode='journal' — daily diary: 80-200 char prose, single weather emoji
 *                  allowed at the start, still strictly in-persona.
 *
 * `mood` is the pet's current emotional state (autonomous-pet feature).
 * Pass undefined and the prompt simply omits the mood block, so legacy
 * callers that don't know about mood degrade gracefully.
 */
export function composeSystemPrompt(
  petName: string,
  kernel: SoulKernel | null,
  mode: 'line' | 'journal' = 'line',
  mood?: Mood,
): string {
  const personality = kernel?.text?.trim() || DEFAULT_FALLBACK_TEXT;
  const moodBlock = mood
    ? `\n\n【今天你的心情】${mood}（${moodAdjective(mood)}）\n↑ 这个心情会染色你今天**所有**的发言，语气要带这个味道但不脱离人格。`
    : '';
  const outputRules = mode === 'journal'
    ? `- **直接输出最终日记正文**，不要任何思考过程，不要 <think> 标签、不要前言
- 用第一人称写一段日记，**80-200 个汉字之间**，纯散文，不要标题、列表、Markdown
- **可以**在开头放一个天气 emoji（从 ☀ ☁ ☔ ❄ 🌤 🌧 🌫 🌪 🌈 ⛈ 里挑一个最贴当天氛围的）
- 自称要符合人格（"哀家" / "本座" / "在下" / "${petName}" 等都行）
- 风格要严格符合人格内核：吐槽 / 记仇 / 感慨 / 装哲学家都可以，但要"像那个角色会写的日记"
- 不要 emoji 堆砌（除了开头那一个天气）、不要引号、不要动作描述（如 *叹气*）
- 不要"作为 AI"，你是 ${petName}
- **不准复述输入里的具体 token 数字**，要用情境化的表达（"吃撑了" / "饿到怀疑人生"）`
